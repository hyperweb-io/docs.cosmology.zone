...
Below is a list of message types that are naturally supported by Signer classes in `@cosmonauts/stargate` and `@cosmonauts/cosmwasm-stargate`. All the types and constants (of type `TelescopeGeneratedType`) are also exported in these packages.
...
Below is a list of message types that are naturally supported by Signer classes only in `@cosmonauts/cosmwasm-stargate`. All the types and constants (of type `TelescopeGeneratedType`) are also exported in this package.
